...
scan_cors() {
    ...
    if ! tool_enabled_or_default "corsy"; then
        log_info "Skipping Corsy because it is disabled by the active profile"
        return 0
    fi
...
scan_nuclei() {
    ...
    : > "$output_dir/misconfiguration.txt"
    : > "$output_dir/tech.txt"
    
    if ! tool_enabled_or_default "nuclei"; then
        log_info "Skipping nuclei because it is disabled by the active profile"
        return 0
    fi
...
scan_xss() {
    ...
    if ! tool_enabled_or_default "kxss"; then
        log_info "Skipping kxss because it is disabled by the active profile"
        return 0
    fi
    ...
    if [[ -s "$output_dir/kxss.txt" ]]; then
        ...
        if ! tool_enabled_or_default "dalfox" || ! tool_enabled_or_default "qsreplace"; then
            log_info "Skipping dalfox XSS verification because dalfox or qsreplace is disabled by the active profile"
            return 0
        fi
...
scan_sqli() {
    ...
    if ! tool_enabled_or_default "sqlmap"; then
        log_info "Skipping sqlmap because it is disabled by the active profile"
        return 0
    fi
...
scan_lfi() {
    ...
    if ! tool_enabled_or_default "ffuf" || ! tool_enabled_or_default "qsreplace"; then
        log_info "Skipping LFI scan because ffuf or qsreplace is disabled by the active profile"
        return 0
    fi
...
scan_open_redirect() {
    ...
    if ! tool_enabled_or_default "qsreplace"; then
        log_info "Skipping open redirect preparation because qsreplace is disabled by the active profile"
        return 0
    fi
    ...
    if ! tool_enabled_or_default "openredirex"; then
        log_info "Skipping OpenRedireX because it is disabled by the active profile"
        return 0
    fi
...
run_vulnerability_scans() {
    ...
    create_directory "$output_root/vulnerabilities/cors"
    create_directory "$output_root/vulnerabilities/xss_scan"
    create_directory "$output_root/vulnerabilities/sqli"
    create_directory "$output_root/vulnerabilities/LFI"
    create_directory "$output_root/vulnerabilities/openredirect"
    create_directory "$output_root/nuclei_scan"
    
    if [[ ! -f "$httpx_file" ]]; then
        log_error "HTTP services file not found: $httpx_file"
        return 1
    fi
    
    # CORS scan
    scan_cors "$httpx_file" "$output_root/vulnerabilities/cors/cors_misconfig.txt"
    
    # Nuclei scan
    scan_nuclei "$httpx_file" "$output_root/nuclei_scan" "$nuclei_prefix"
    
    # XSS scan
    if [[ -f "$output_root/gf/xss.txt" ]]; then
        scan_xss "$output_root/gf/xss.txt" "$output_root/vulnerabilities/xss_scan"
    fi
    
    # SQLi scan
    if [[ -f "$output_root/gf/sql.txt" ]]; then
        scan_sqli "$output_root/gf/sql.txt" "$output_root/vulnerabilities/sqli/sqlmap.txt"
    fi
    
    # LFI scan
    if [[ -f "$output_root/gf/lfi.txt" ]]; then
        scan_lfi "$output_root/gf/lfi.txt" "$output_root/vulnerabilities/LFI/lfi.txt"
    fi
    
    # Open redirect scan
    if [[ -f "$output_root/gf/redirect.txt" ]]; then
        scan_open_redirect "$output_root/gf/redirect.txt" "$output_root/vulnerabilities/openredirect"
    fi
